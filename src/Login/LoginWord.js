...
export default class LoginWord extends React.Component {
    ...
    async LoadLogin () {
        try {
            ...
            let data = await Utils.postJSON(Utils.size.url + '/api/auth/loginWord' , formData);
            console.log(data);
            if (Number(data.code) === 0) {
                Loading.hidden();
                AsyncStorage.setItem('pmId', data.result.pm_id);
